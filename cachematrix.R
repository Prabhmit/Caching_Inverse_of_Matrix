...
cacheSolve <- function(x, ...) {
  ...
  if(!is.null(inverse_matrix)) {                                  # checks if inverse_matrix is not null
    message("getting cached matrix inverse")                      # display message and returns cached inverse_matrix
    return(inverse_matrix)    
  }
  data <- x$get()                                                 # gets matrix data
  inverse_matrix <- solve(data, ...)                              # using solve to calculate inverse
  x$set_inverse(inverse_matrix)                                   # sets inverse_matrix
  inverse_matrix                                                  #returns inverse_matrix 
}
